...
# tạo của sổ tên là count và có thể tự thay đổi kích thước thủ công
cv2.namedWindow("count", cv2.WINDOW_NORMAL)
# ham khu nhieu o mien tan so


def denoisePeriodic(img, size_filter=2, diff_from_center_point=1000, size_thresh=2):

    h, w = img.shape[:]
    img_float32 = np.fft.fft2(img)  # tinh biến đỏi furrier 2 chiều rời rạc
    # dich chuyen tan so 0 ve trung tam cua pho anh
    fshift = np.fft.fftshift(img_float32)  # dich chuyen 0 ra tam cua quang pho
    ...
    kernel = np.ones((2*size_filter+1, 2*size_filter+1),
                     np.float32)/((2*size_filter+1)*(2*size_filter+1)-1)  # tạo nhân chập kich thuoc 2*sixe_filter^2
    ...
    for i, item in enumerate(index[0]):
        ...
        # print("value ", value)
        matrix = np.copy(furier_tr[max(0, index[0][i]-size_filter):min(h, index[0][i] +
                         size_filter+1), max(0, index[1][i]-size_filter):min(w, index[1][i]+size_filter+1)])
...
def count_general(img):
    ...
    ero_all = cv2.flip(blurred, -1)  # quay anh quanh ca 2 truc x, y
    ero_ver = cv2.flip(blurred, 0)  # quay ảnh quanh truc x
    ero_hor = cv2.flip(blurred, 1)  # quay ảnh quanh truc y

    new_1 = np.concatenate((ero_all, ero_ver, ero_all), axis=1)  # nối ảnh
    ...
    clahe = cv2.createCLAHE(clipLimit=5, tileGridSize=(
        int(w/50), int(h/50)))  # ngưỡng ghan cho do tuong phan
    ...
    thres1 = cv2.adaptiveThreshold(
        cl1, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 55, -12)
    ...
    for i, c in enumerate(contours):
        ...
        count += 1
        cv2.rectangle(img_ori, (int(boundRect[0]), int(boundRect[1])), (int(
            boundRect[0]+boundRect[2]), int(boundRect[1]+boundRect[3])), (0, 255, 0), 1)
...
def count_normal(img):
    ...
    new_ero = np.concatenate((new_1, new_2, new_3), axis=0)
    # nhi phan anh cuc bo
    new_ero = cv2.adaptiveThreshold(
        new_ero, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 55, -5)
    ...
    # co gian anh de loai nhieu va tach object lien nhau
    thres = cv2.erode(thres1, kernel, iterations=1)  # co
    # cv2.imshow("thres_er",thres)
    thres = cv2.dilate(thres, kernel, iterations=1)
    # cv2.imshow("thres_dia",thres)
...
for path in glob.glob("rice/*"):
    ...
    # print(thres)
    img = denoisePeriodic(img_gray, size_filter=int(
        thres/2), diff_from_center_point=50, size_thresh=thres)
